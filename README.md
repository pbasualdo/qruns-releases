...
IT Quick Runbooks is a desktop application built with Electron, React, and Vite designed to streamline the execution of operational procedures. It allows teams to aggregate runbooks from local folders and Git repositories, view them in a clean UI, and execute steps efficiently.

![IT Quick Runbooks Screenshot](https://via.placeholder.com/800x450?text=IT+Quick+Runbooks+Preview) *Add a screenshot here later*
...
2. Run the installer `IT Quick Runbooks Setup x.x.x.exe`.
3. Launch the app!
4. (Optional) **Enable Auto-Updates**: Run `scripts/Install-Certificate.ps1` to trust the self-signed certificate.

## 🔄 Updates

You have two options to keep IT Quick Runbooks up to date:
