...
function serializeToMarkdown(runbook) {
  const { steps, sourcePath, format: format2, ...meta } = runbook;
  ...
  try {
    ...
    sources.forEach((dir) => {
      if (fs$1.existsSync(dir)) {
        ...
        files.forEach((file2) => {
          if (file2.endsWith(".json") || file2.endsWith(".md")) {
            const rb = parseRunbookFile(path.join(dir, file2));
...
ipcMain.handle("save-runbook", async (_, runbook) => {
  try {
    ...
    if (runbook.format === "md") {
      ...
      const { sourcePath, format: format2, ...cleanRunbook } = runbook;
...
ipcMain.handle("clone-repository", async (_, url, options2 = {}) => {
  try {
    ...
    return new Promise((resolve) => {
      ...
      exec(command, execOptions, (error2, stdout, stderr) => {
        let success = false;
        if (interactive) {
        ...
        if (success) {
          ...
          if (!fs$1.existsSync(qrunPath)) {
            try {
              fs$1.rmSync(targetDir, { recursive: true, force: true });
            } catch (e) {
...
ipcMain.handle("download-template", async (_, format2) => {
  if (!win) return { success: false, error: "No window" };
  try {
    const defaultName = format2 === "md" ? "template.md" : "template.json";
    const content = format2 === "md" ? TEMPLATE_MD : TEMPLATE_JSON;
    const { filePath } = await dialog.showSaveDialog(win, {
      defaultPath: defaultName,
      filters: [{ name: format2.toUpperCase(), extensions: [format2] }]
    });
    if (filePath) {
      fs$1.writeFileSync(filePath, content, "utf-8");
      return { success: true };
